...
from .routes_jobs import USER_STATUS_RADAR
...
def stage_urls(db, urls: List[str], source: Optional[str] = None) -> Dict[str, int]:
    ...
    marked ``user_status: radar``. The mark survives promotion (``stage_jobs``
    rewrites only the described fields) and is applied to the job on commit, so
    a URL pasted here arrives in the list already on the user's radar.
...
def commit():
    ...
    for raw_id in request.form.getlist("select"):
        ...
        # A record queued as a bare URL carries a radar mark (see ``stage_urls``).
        # Only apply it when the job is created: an existing job's user_status is
        # the user's own, so a re-import must not knock it back from applied.
